...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%             Event Generation (alphas, periods)                          %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
events_list = floor(total_time_vec./periods_list); % [2400 1200 800 600 480 floor(342.85)]  


effform = zeros(num_periods,n_alphas);      %%%%%%%%%%%% Build effform %%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%                            Many for-loops                               %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
 % Start with one efficiency matrix
    
for l = 1: num_periods % Choose a period thus a n_events./ build jitter 
    n_events = events_list(l); 
events = randi(10,1, n_events); 
rng(24);% To replicate
t = linspace(starting_time, total_time+starting_time, n_events); % I have 
%to make a one second delay here, so that the index is not 0 in
%func_highRes to disobey the rule that "Subscript indices must either be 
...
twodsimalphas = reshape(sim_alphas_estimates,n_sims,n_models); %n_models = n_Ymodels.
for hh = 1:n_models
    sum_var = 0;
    for mm = 1:n_sims
...
colormap(cmap);
imagesc(alphas, periods_list, effform); 
...
ylabel('Periods');
% Create title
title({'Estimated Experiment Design Efficiency for simulated data with different'
    'values experiment periods across true parameter value alpha'});